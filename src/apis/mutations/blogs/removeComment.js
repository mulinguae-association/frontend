...
export const useRemoveCommentMutation = (blogId, parentComment) => {
  ...
  return useMutation(({ commentId }) => refuseComment(commentId, blogId), {
    onMutate: async ({ commentId }) => {
      ...
      const previousComments = await queryClient.cancelQueries([
        "comments",
        blogId,
      ]);
      const previousReplies = await queryClient.cancelQueries([
        "remaining-replies",
        parentComment,
      ]);
      // Optimistically update the comments
      queryClient.setQueryData(["comments", blogId], (oldData) =>
        updateComments(oldData, commentId),
      );
      if (parentComment !== null) {
        queryClient.setQueryData(
          ["remaining-replies", parentComment],
          (oldData) => updateReplies(oldData, commentId),
        );
      }

      return { previousComments, previousReplies };
    },
    onSuccess: (res) => handleSuccess(res, blogId, queryClient),
    onError: (error, _, context) =>
      handleErrorCase(error, context, blogId, parentComment, queryClient),
  });
};
...
const updateComments = (oldData, commentId) => {
  ...
  const newPages = oldData.pages.map((page) => {
    ...
    const updatedComments = page.acceptedComments.filter(
...
const updateReplies = (oldData, commentId) => {
  ...
  const newPages = oldData.pages.map((page) => {
    const updateComments = {
      ...page,
      remainingReplies: page.remainingReplies.filter(
...
const handleSuccess = (res, blogId, queryClient) => {
