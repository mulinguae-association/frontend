...
function ForgotPassword() {
  ...
  const handleInputChange = (e) => {
    ...
    const error = !isVAlidEmail(value)
      ? global("Error_Invalid_Email_Format")
      : "";
  ...
  const isBtnDisabled =
    !formState.email || formState.error || isBtnLoading["resetEmail"];
  ...
  return (
    <main className="auth_form forgot-password">
          ...
          {formState.emailSent ? (
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="group">
                ...
                />
                {formState.error && (
                  <p className="error-msg">{formState.error}</p>
                )}
              </div>
              <button
                type="submit"
      ...
      <p>
        {t("noAccount")}{" "}
        <Link to={`/${i18next.language}/register`}>{t("signUpLink")}</Link>
      </p>
    </main>
  );
}
