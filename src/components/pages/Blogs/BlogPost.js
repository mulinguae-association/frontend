...
import { getAcceptedComments, getRemainingAcceptedReplies } from "../../../apis/blog-api";
...
const BlogPost = ({ blog, list }) => {
  ...
  const { data, fetchNextPage, isFetching, hasNextPage } = useInfiniteQuery(
    ...
    ({ pageParam = 1 }) => getAcceptedComments({ blogId: blog._id, pageParam }),
    {
      getNextPageParam: (lastPage, allPages) => {
        return lastPage.acceptedComments.length ? allPages.length + 1 : undefined;
      }
    }
  );
  ...
  const { data: lastReplyData } = useInfiniteQuery(
    ...
    ({ pageParam = 1 }) => getRemainingAcceptedReplies({ parentCommentIds: lastCommentId, pageParam, limit: 1 }),
    {
      getNextPageParam: (lastPage, allPages) => {
        return lastPage.remainingReplies.length ? allPages.length + 1 : undefined;
      },
