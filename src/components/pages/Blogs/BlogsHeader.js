  ...
  return (
    <div className='blogs_header'>
      <div className="links button-font">
        <Link to={`create-new-blog`}>{`+ ${t("createABlog.name")} ${t("createABlog.special")}`}</Link>
          ...
          <div className='user_info'>
            ...
            {showUserMenu &&
              <React.Suspense fallback={
                <span
                  className='user_menu'
                  style={isAr_Ur ? { left: 0, right: "unset" } : { right: 0, left: "unset" }}>
                  <BiLoaderCircle className='spin-loader' />
                </span>}>
                <UserMenu handleLogout={handleLogout} setShowMenuUser={setShowMenuUser} />
              </React.Suspense>}
          </div>
        ) :
