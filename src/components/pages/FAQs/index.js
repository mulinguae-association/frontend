...
const FAQs = () => {
  ...
  const validation = () => {
    const { email, fullName, details, affiliation, affiliationOther, type, operatingSystem, operatingSystemOther, browser, browserOther, screenshot } = formData;
    ...
    if (affiliation === t("affiliationSelect.options.other") && !affiliationOther) {
      notifyError(t("errorMessages.otherAffiliationEmpty"));
      return false;
    } else if (browser === t("bugComponent.browserOptions.other") && !browserOther) {
      notifyError(t("errorMessages.otherBrowserEmpty"));
      return false;
    } else if (operatingSystem === t("bugComponent.browserOptions.other") && !operatingSystemOther) {
      notifyError(t("errorMessages.otherOSEmpty"));
      return false;
    }

    if (type === t("typeSelect.options.bug") && (!browser || !operatingSystem || !screenshot)) {
    ...
    <main className='FAQs'>
      <div className='container'>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button disabled={isBtnLoading["FAQS_button"]} type="submit">{isBtnLoading["FAQs_button"] ?
            t("form.submittingButton") : t("form.submitButton")}
          </button>
